...
#define MEM1_MAX_SIZE           428 * 1024                      /* 最大管理内存 448K,H7的AXI内存总共512KB */
#define MEM1_ALLOC_TABLE_SIZE   MEM1_MAX_SIZE / MEM1_BLOCK_SIZE /* 内存表大小 */

/* mem2内存参数设定.mem2是外部的SDRAM内存 */
#define MEM2_BLOCK_SIZE         64                              /* 内存块大小为64字节 */
#define MEM2_MAX_SIZE           100 * 1024                    /* 最大管理内存28912K,外扩SDRAM总共32MB,LTDC占了2MB,还剩30MB. */
...
void *lv_mymalloc(uint32_t size);	/* 内存分配(lvgl调用) */
void lv_myfree(void *ptr); /* 内存释放(lvgl调用) */
void *lv_myrealloc(void *ptr, uint32_t size); /* 内存重分配(lvgl调用) */
