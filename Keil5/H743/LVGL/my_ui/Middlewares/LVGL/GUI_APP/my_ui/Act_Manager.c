...
static lv_obj_t* Bright_mask = NULL;        //亮度遮罩（测试）
...
void act_manager_init(){
    ...
    act_manager_register(test2_def_get());
...
void act_manager_switch_to(app_def_t* new_app, void* user_data) {
    ...
    // 加载新屏幕
    lv_scr_load_anim(new_activity->screen, LV_SCR_LOAD_ANIM_FADE_ON, 300, 0, false);
}
...
void act_manager_go_home(void) {
    ...
    lv_scr_load_anim(home_activity->screen, LV_SCR_LOAD_ANIM_OVER_TOP, 300, 0, true);
...
static void edge_gesture_event_cb(lv_event_t* e) {
    ...
    /* 按下后没有松开 */
        ...
        if (!G_gesture_state.is_pressing) return;
        ...
        if (G_gesture_state.target_hotzone == G_top_hotzone && dy > EDGE_GESTURE_THRESHOLD && popup_is_pd_menu_active() == false) {
            ...
            params.initial_volume = 50; // 假设音量是50
#ifndef NDEBUG
    ...
    /* 当手指抬起时 */
        ...
        if (popup_is_pd_menu_active() == false) {//下拉菜单不存在
            // 事件判断
