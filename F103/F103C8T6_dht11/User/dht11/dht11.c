...
void DHT11_IO_OUT (void){ //端口变为输出
	...
	HAL_GPIO_Init(DHT11_DA_Port, &GPIO_InitStruct);
}
...
void DHT11_RST (void){ //DHT11端口复位，发出起始信号（IO发送）
	DHT11_IO_OUT();
	HAL_GPIO_WritePin(DHT11_DA_Port,DHT11_DA_Pin, GPIO_PIN_RESET);
	HAL_Delay(20); //拉低至少18ms
	HAL_GPIO_WritePin(DHT11_DA_Port,DHT11_DA_Pin, GPIO_PIN_SET);
...
uint8_t Dht11_Check(void){ //等待DHT11回应，返回1:未检测到DHT11，返回0:成功（IO接收）
	uint8_t retry=0;
    DHT11_IO_IN();//IO到输入状态
    while (HAL_GPIO_ReadPin(DHT11_DA_Port,DHT11_DA_Pin)&&retry<100){//DHT11会拉低40~80us
        retry++;
        delay_us(1);
    }
    if(retry>=100)return 1; else retry=0;
    while (!HAL_GPIO_ReadPin(DHT11_DA_Port,DHT11_DA_Pin)&&retry<100){//DHT11拉低后会再次拉高40~80us
        retry++;
        delay_us(1);
    }
...
uint8_t Dht11_ReadBit(void){ //从DHT11读取一个位 返回值：1/0
	uint8_t retry=0;
    while(HAL_GPIO_ReadPin(DHT11_DA_Port,DHT11_DA_Pin)&&retry<100){//等待变为低电平
        retry++;
        delay_us(1);
    }
    retry=0;
    while(!HAL_GPIO_ReadPin(DHT11_DA_Port,DHT11_DA_Pin)&&retry<100){//等待变高电平
        retry++;
        delay_us(1);
    }
    delay_us(40);//等待40us	//用于判断高低电平，即数据1或0
    if(HAL_GPIO_ReadPin(DHT11_DA_Port,DHT11_DA_Pin))return 1; else return 0;
}
